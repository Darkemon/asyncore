...
version(Windows) {
	...
	public import deimos.uv_private.win; 
} else {
	public import core.sys.posix.netdb; 
	import core.sys.posix.sys.stat;
	alias stat_t uv_statbuf_t;
	public import deimos.uv_private.unix; 
}
...
package alias ptrdiff_t ssize_t;
 ...
 */
immutable(char)* uv_version_string();
 ...
 *    pending events.
 */
int uv_run(uv_loop_t*, uv_run_mode mode = uv_run_mode.UV_RUN_DEFAULT);
 ...
 */
alias void function (uv_stream_t* stream, ssize_t nread, uv_buf_t buf) uv_read_cb;
 ...
 */
alias void function(uv_pipe_t* pipe, ssize_t nread, uv_buf_t buf, uv_handle_type pending) uv_read2_cb;

alias void function(uv_write_t* req, int status) uv_write_cb;
alias void function(uv_connect_t* req, int status) uv_connect_cb;
alias void function(uv_shutdown_t* req, int status) uv_shutdown_cb;
alias void function(uv_stream_t* server, int status) uv_connection_cb;
alias void function(uv_handle_t* handle) uv_close_cb;
alias void function(uv_poll_t* handle, int status, int events) uv_poll_cb;
alias void function(uv_timer_t* handle, int status) uv_timer_cb;
/* TODO: do these really need a status argument? */
alias void function(uv_async_t* handle, int status) uv_async_cb;
alias void function(uv_prepare_t* handle, int status) uv_prepare_cb;
alias void function(uv_check_t* handle, int status) uv_check_cb;
alias void function(uv_idle_t* handle, int status) uv_idle_cb;
alias void function(uv_process_t*, int exit_status, int term_signal) uv_exit_cb;
...
alias void function(uv_work_t* req, int status) uv_after_work_cb;
alias void function(uv_getaddrinfo_t* req, int status, addrinfo* res) uv_getaddrinfo_cb;
...
*/
alias void function(uv_fs_event_t* handle, const char* filename, int events, int status) uv_fs_event_cb;

alias void function (uv_fs_poll_t* handle, int status, const uv_stat_t* prev, const uv_stat_t* curr) uv_fs_poll_cb;

alias void function(uv_signal_t* handle, int signum) uv_signal_cb;
 ...
 */
alias void function (uv_udp_send_t* req, int status) uv_udp_send_cb;
 ...
 */
alias void function (uv_udp_t* handle, ssize_t nread, uv_buf_t buf, sockaddr* addr, uint flags) uv_udp_recv_cb;
 ...
 *  0 on success, -1 on error.
 */
int uv_udp_send6(uv_udp_send_t* req, uv_udp_t* handle, uv_buf_t bufs[], int bufcnt, sockaddr_in6 addr, uv_udp_send_cb send_cb);
